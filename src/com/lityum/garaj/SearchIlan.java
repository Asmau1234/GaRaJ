...
public class SearchIlan extends Activity implements OnItemSelectedListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		activity.findViewById(android.R.id.content).setBackgroundColor(
				Color.LTGRAY);
		Intent intent = SearchIlan.this.getIntent();
		Bundle extras = intent.getExtras();
		ilanIntent = extras.getString("Pano");
		ilanIntent2 = extras.getString("Sehir");
	...
	class SearchIlanTask extends AsyncTask<Void, Void, Void> {
		...
		protected void onPreExecute() {
			...
			if (pDialog == null) {
				...
				pDialog = new ProgressDialog(SearchIlan.this);
		...
		protected Void doInBackground(Void... arg0) {
			...
			pairs.add(new BasicNameValuePair("pano", ilanIntent));
			pairs.add(new BasicNameValuePair("sehir", ilanIntent2));
			...
			if (jsonStr != null) {
				...
				try {
					...
					for (int i = 0; i < ilans.length(); i++) {
						...
						ilanlar = new RowItem();
						ilanlar.setDate(c.optString("fiyat")); // using setDate
																// in
																// place of
																// Fiyat so as
																// not to
		...
		protected void onPostExecute(Void result) {
			if (pDialog.isShowing())
				pDialog.dismiss();
			adapter = new CustomListView(SearchIlan.this, R.layout.list_item_1,
					ilanList);
			...
			myListViewClicked = new OnItemClickListener() {
				...
				public void onItemClick(AdapterView<?> parent, View view,
					...
					Intent i = new Intent(SearchIlan.this, IlanDisplay.class);
	...
	class IlanButtonsTask extends AsyncTask<Void, Void, Void> {
		...
		protected void onPreExecute() {
			...
			if (pDialog == null) {
				// Showing progress dialog
				pDialog = new ProgressDialog(SearchIlan.this);
				pDialog.setMessage("Lutfen Bekle...");
				pDialog.setCancelable(false);
				pDialog.show();
			} else {
				pDialog.dismiss();
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		getMenuInflater().inflate(R.menu.search_ilan, menu);
		return true;
	}
	...
	public void showProfile() {
		...
		Intent profile = new Intent(SearchIlan.this, Profile.class);
		startActivity(profile);

	}

	public void showMessages() {
		Intent messages = new Intent(SearchIlan.this, Messages.class);
		startActivity(messages);

	}

	public void showFriends() {
		Intent friends = new Intent(SearchIlan.this, Friends.class);
